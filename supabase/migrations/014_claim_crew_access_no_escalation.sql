-- Remove privilege escalation: only onboarded/hired applicants may claim crew via RPC.
-- The previous ELSE branch promoted any authenticated user without a matching applicant row.
...
comment on function public.claim_crew_access_for_me() is
  'Links auth.uid() to crew only when an onboarded/hired applicant row exists for their email. Idempotent for existing cleaners.';
